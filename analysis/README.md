...
| 指标 | 一句话描述 |
| --- | --- |
| **Missense** | 错义突变，指单个氨基酸发生改变，可能导致蛋白功能增强、减弱或改变。 |
| **Truncating** | 截断型突变，通常包括无义突变、移码突变等，多提示蛋白功能丧失。 |
| **Inframe** | 框内插入或缺失，不改变阅读框，但会导致蛋白局部氨基酸改变。 |
| **Splice** | 剪接位点突变，可能影响 mRNA 正常剪接并导致异常蛋白产生。 |
| **OtherMut** | 其他非同义突变，指未归入上述主要类别的潜在功能性突变。 |
| **AMP** | 拷贝数扩增，表示该基因所在区域总拷贝数升高，可能提示癌基因激活。 |
| **DEL** | 拷贝数缺失，表示该基因所在区域拷贝数降低，可能提示抑癌基因丢失。 |
| **HomDel** | 纯合缺失，表示该基因区域双拷贝均丢失，是较强的功能丧失证据。 |
| **LOH** | 杂合性缺失，表示一个等位基因丢失，常与突变共同支持双等位基因失活。 |
| **SV** | 结构变异，表示 Manta 检测到的断点影响该基因区域，可能提示基因破坏或调控异常。 |
| **Altered SCCC frequency (%)** | 该基因在 SCCC 样本中发生任意 alteration 的比例，包括突变、CNV、LOH 和 SV。 |
| **No. altered SCCC** | 该基因发生任意 alteration 的 SCCC 样本数，占总样本数的形式如 `8/30`。 |
